...
struct GuiWinManage{
    uint8_t pWin;
    const lkdWin *winStack[GUIWINMANAGE_NUM];
};
...
typedef struct lkdMenuNode{
    uint16_t nodeId;            /* 节点号 */
    uint8_t *name;              /* 节点名称 */
    const struct lkdMenuNode *pBro;   /* 下一个兄弟节点号 */
    const struct lkdMenuNode *pSon;   /* 子节点号 */
    const void *user;
}lkdMenuNode;
...
typedef struct{
    ...
    MenuStack *stack;   /* 菜单栈 */
    const lkdMenuNode *Root;  /* 菜单根节点 */
    void (* MenuDealWithFun)(void);/* 菜单处理函数 */

    /* 当前节点绘画临时保存 */
    lkdCoord cx;        /* 起始坐标 */
    lkdCoord cy;
    const lkdMenuNode *pDraw; /* 绘画节点 */
