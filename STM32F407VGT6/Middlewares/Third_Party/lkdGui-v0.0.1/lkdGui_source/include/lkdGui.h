...
void GuiRowTextPlus(lkdCoord x, lkdCoord line,uint16_t wide, FontFlag flag,uint8_t *str);
/* 设置当前要使用的字体指针 */
...
/* 添加窗口 */
int8_t GuiWinAdd(const lkdWin *pWindow);
/* 显示窗口 */
void GuiWinDisplay(void);
/* 删除顶层窗口 */
void GuiWinDeleteTop(void);
/* 获取顶层窗口 */
const lkdWin *GuiGetTopWin(void);
...
/*绘制的节点*/
void DrawNodes(lkdMenu *pM, const lkdMenuNode *pthisNode,lkdCoord x,lkdCoord beginy,uint8_t index,uint8_t cNum);
/* 清全部屏幕 */
