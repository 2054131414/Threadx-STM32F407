...
#include "basic_os.h"
/* Gui显示缓冲区 */
static uint8_t  lkdGuiBuff[GUIXMAX][(GUIYMAX+7)>>3] MY_BSS_CCMRAM;
static uint8_t  lkdGuiBuff_bak[GUIXMAX][(GUIYMAX+7)>>3] MY_BSS_CCMRAM;
/* 行位获取数组 */
...
void GuiDrawPoint(lkdCoord x, lkdCoord y, lkdColour color)
{
  /* 添加用户代码 */
    ...
    bx=y%8;
    temp=1<<(7-bx);
    if(color)lkdGuiBuff[x][pos]|=temp;
