...
/**
  ...
  */
const lkdMenuNode *GetpNodeFromIndex(lkdMenu *pM)
{
	const lkdMenuNode *tpNode = pM->pDraw;
	uint8_t nodeNum = 0;
	while(tpNode != NULL){
		if(nodeNum == pM->index){
			return tpNode;
...
/**
  ...
  *@retval None
  */
void DrawNodes(lkdMenu *pM,const lkdMenuNode *pthisNode,lkdCoord x,\
...
/**
  ...
  */
static uint8_t GuiMenuNodesDisplay(lkdMenu *pM, const lkdMenuNode *node)
{
...
/**
  ...
  *@retval 节点指针
  */
const lkdMenuNode *GuiMenuGetCurrentpNode(lkdMenu *pM)
{
